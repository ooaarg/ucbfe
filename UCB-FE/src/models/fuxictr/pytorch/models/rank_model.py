...
class BaseModel(nn.Module):
    ...
    def add_loss(self, return_dict, y_true):
        # print(torch.unique(return_dict["y_pred"]).shape)
    ...
    def reset_parameters(self):
        def default_reset_params(m):
            ...
            if type(m) in [nn.Linear, nn.Conv1d]:
                # print(m)
                # if hasattr(self, 'model_id') and self.model_id == 'xDeepFM':
    ...
    def train_step(self, batch_data):
        ...
        # print(torch.unique(return_dict["y_pred"]).shape, torch.min(return_dict["y_pred"]).item(), torch.max(return_dict["y_pred"]).item())
    ...
    def predict(self, data_generator):
        ...
        with torch.no_grad():
            ...
            for batch_data in data_generator:
                ...
                y_pred.extend(return_dict["y_pred"].data.cpu().numpy().reshape(-1))
            y_pred = np.array(y_pred, np.float64)
            # print(y_pred, y_pred.shape, np.sum(y_pred))
            return y_pred
